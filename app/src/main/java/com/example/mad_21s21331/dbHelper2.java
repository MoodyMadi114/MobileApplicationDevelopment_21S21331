...
public class dbHelper2 extends SQLiteOpenHelper {
    ...
    public Cursor MohViewAll(){
        SQLiteDatabase db=this.getReadableDatabase();
        Cursor cursor=db.rawQuery("SELECT  * FROM "+TableName,null);
        return cursor;
    }
    public boolean UpdateOcc(String Occ,String PN){
        SQLiteDatabase dbMadi1=this.getWritableDatabase();
        ContentValues contentValues=new ContentValues();
        contentValues.put(FIRST_COLUMN,Occ);
        dbMadi1.update(TableName,contentValues,"PhoneNumber=?",new String[]{PN});
        return true;
    }
    public Integer MadiDeleteRecord(String PN){
        SQLiteDatabase db=this.getWritableDatabase();
        return db.delete(TableName,"PhoneNumber=?",new String []{PN});
    }
}